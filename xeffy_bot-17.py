...
def register_airdrop(session_token, discord_handle, wallet_address):
    payload = {
        "acceptedTerms": True,
...
async def run_account(session_string, index, tele_links, mode="full", airdrop_data=None, xhandles=None):
    ...
    if mode == "airdrop":
        ...
        # Ambil x_handle dari status (opsional)
        x_handle = None
        if status and status.get("xFollow"):
            x_handle = status["xFollow"].get("sourceUsername")
        ...
        ok = register_airdrop(session_token, discord, wallet)
