...
async def run_cek_balance(session_string, idx, total):
    ...
    try:
        dashboard = get_dashboard(access_token)
        balance_usdc = dashboard.get("balance_usdc", "?")
        today_pnl = dashboard.get("today_pnl_usdc", "?")
        print(f"{tag} 💰 Balance: ${balance_usdc} USDC")
        print(f"{tag} 📈 Today PnL: ${today_pnl} USDC")
...
async def select_mode_and_accounts():
    ...
    print("  9. Cek balance")
    mode = input("Pilih mode (1/2/3/4/5/6/7/8/9): ").strip()
