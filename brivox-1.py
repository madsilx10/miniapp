...
# ── HTTP helper (urllib bawaan, tanpa perlu install lib tambahan) ──────────────
...
# ── API: Login ────────────────────────────────────────────────────────────────
async def login(init_data: str, idx: int) -> str | None:
    try:
        headers = {
...
# ── API: Balance ──────────────────────────────────────────────────────────────
async def get_balance(token: str, idx: int):
    try:
        data = await http_get(f"{BASE_URL}/api/crypto/user/crypto-assets/list", build_headers(token))
...
# ── API: Buy NFT ──────────────────────────────────────────────────────────────
async def buy_nft(token: str, idx: int) -> bool:
    try:
        data = await http_post(f"{BASE_URL}/api/deal/user/load/create", build_headers(token), {"deal_id": DEAL_ID})
...
# ── API: Spin Info ────────────────────────────────────────────────────────────
async def get_spin_info(token: str, idx: int) -> dict | None:
    try:
        data = await http_get(f"{BASE_URL}/api/user/user/user-lottery/info", build_headers(token))
        spin = data.get("info", {})
...
# ── API: Do Spin ──────────────────────────────────────────────────────────────
async def do_spin(token: str, idx: int):
    try:
        data = await http_post(f"{BASE_URL}/api/user/user/user-lottery/create", build_headers(token), {})
        if data.get("type"):
...
async def process_account(session_string: str, idx: int, spin_mode: str):
    ...
    # 2. Login → dapat token
    token = await login(init_data, idx)
    if not token:
        return
    await asyncio.sleep(DELAY)

    # 3. Cek balance awal
    await get_balance(token, idx)
    await asyncio.sleep(DELAY)

    # 4. Buy NFT
    await buy_nft(token, idx)
    await asyncio.sleep(DELAY)

    # 5. Spin
    if spin_mode == "auto":
        spin_info = await get_spin_info(token, idx)
        await asyncio.sleep(DELAY)
        if spin_info:
