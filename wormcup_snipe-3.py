...
APIFOOTBALL_KEY = "901541ad83a200e3edc67eff19dc5799"
APIFOOTBALL_BASE = "https://v3.football.api-sports.io"

_team_strength_cache = {}  # {team_code: strength_score}
...
def get_team_strength(team_code):
    """
    Ambil strength score tim dari API-Football standings.
    Return 0-100 scale (100 = strongest).
    """
    if team_code in _team_strength_cache:
        return _team_strength_cache[team_code]
    
    try:
        # Request standings dari World Cup 2026
        r = requests.get(
        ...
        if not data.get("response"):
            return 50  # default middle strength
        
        # Flatten all groups
        all_teams = []
        for group in data["response"]:
            if "standings" in group and group["standings"]:
                all_teams.extend(group["standings"][0])  # First group in standings array
        
        # Find team by code
        for team_entry in all_teams:
            if team_entry.get("team", {}).get("name", "").upper() == team_code.upper():
                # Calculate strength: rank-based (top team = 100, last = 0)
                rank = team_entry.get("rank", 50)
                strength = max(0, 100 - (rank - 1) * 2)  # Rough formula
                _team_strength_cache[team_code] = strength
                return strength
        
        # Not found, return middle
        return 50
    except Exception as e:
        print(f"  [!] Error fetching team strength: {e}")
        return 50
...
async def do_snipe(match, tokens_per_acc, scores_queue):
    ...
    if scores_queue:
        ...
        # Pakai logic favored dengan team strength dari API-Football
        home_code = match["home"]["code"]
        away_code = match["away"]["code"]
        
        home_strength = get_team_strength(home_code)
        away_strength = get_team_strength(away_code)
        ...
        if strength_diff > 20:
            # Strength gap besar, favorit dominan (60-90% gol)
            fav = round(total * random.uniform(0.6, 0.9))
        else:
            # Strength gap kecil, bisa seri/underdog menang (40-70% gol)
        ...
        # Ambil jersey dari 4 pemain depan tim yang unggul
        athletes = match.get("athletes", [])
        jersey = get_scorer_jersey_from_athletes(athletes, home_code, away_code, hs, as_)
        print(f"  Skor (API-strength): {hs}-{as_} jersey={jersey} (H:{home_strength} A:{away_strength})")
