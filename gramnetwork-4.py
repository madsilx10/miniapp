...
MAX_RETRY        = 10
RETRY_DELAY      = 3
CHANNEL_USERNAME = "Gramnetworkminiapp"
...
# ===================== JOIN CHANNEL =====================
async def join_channel(client: Client, tag: str):
    try:
        chat = await client.get_chat(CHANNEL_USERNAME)
        member = await client.get_chat_member(chat.id, "me")
        if member.status.name not in ("LEFT", "BANNED"):
            print(f"{tag} sudah join @{CHANNEL_USERNAME}")
            return True
    except Exception:
        pass
    try:
        await client.join_chat(CHANNEL_USERNAME)
        print(f"{tag} berhasil join @{CHANNEL_USERNAME}")
        await asyncio.sleep(2)
        return True
    except Exception as e:
        print(f"{tag} gagal join @{CHANNEL_USERNAME}: {e}")
        return False
