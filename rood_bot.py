...
def post_referral(init_data):
    ...
    print(f"    [DEBUG] POST /referral -> status {r.status_code}")
    try:
        print(f"    [DEBUG] response body: {r.json()}")
    except Exception:
        print(f"    [DEBUG] response body (raw): {r.text[:300]}")
...
async def get_init_data(session_string, index):
    async with Client(
        ...
        # Kasih jeda biar background task Pyrogram (handle_updates)
        # kelar dulu sebelum koneksi in-memory ditutup
        await asyncio.sleep(0.5)

        if not tg_web_app_data:
            return None, me

        return urllib.parse.unquote(tg_web_app_data), me
...
async def run_account(session_string, index, mode):
    ...
    init_state = get_state(init_data)
    print(f"    [DEBUG] GET /state -> {'OK' if init_state else 'GAGAL/None'}")
