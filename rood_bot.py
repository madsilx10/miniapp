...
def claim_reward(init_data, captcha_answer, captcha_token):
    ...
    if r.status_code == 200:
        return r.json()
    print(f"    [DEBUG] claim-reward status {r.status_code}: {r.text[:300]}")
    return None
...
async def get_init_data(session_string, index):
    ...
    try:
        ...
        # Tunggu task update internal Pyrogram beres dulu sebelum stop,
        # biar ga ada write ke storage yang udah closed
