...
async def run_account(session_string, index, tele_links, mode="full", airdrop_data=None, xhandles=None):
    ...
    # Cek status airdrop
    if mode == "check_airdrop":
        status = check_airdrop_status(session_token)
        if status:
            reg = status.get("registration")
            if reg and reg.get("discordHandle"):
                print(f"[Akun {index}] ✅ Sudah register - Discord: {reg.get('discordHandle')}, Wallet: {reg.get('walletAddress')[:10]}...")
            else:
                print(f"[Akun {index}] ❌ Belum register")
        else:
            print(f"[Akun {index}] ❌ Gagal cek status")
        return
