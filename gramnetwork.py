    ...
    for attempt in range(1, MAX_RETRY + 1):
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"{tag} verify gagal {r.status_code}, retry {attempt}/{MAX_RETRY}...")
        except Exception as e:
            print(f"{tag} error: {e}, retry {attempt}/{MAX_RETRY}...")
        time.sleep(RETRY_DELAY)
    print(f"{tag} verify gagal setelah {MAX_RETRY} percobaan")
    return False
...
async def process(session_string: str, idx: int, total: int):
    ...
    client = Client(name=f"gram{idx}", session_string=session_string, in_memory=True, no_updates=True)
    await client.start()
