...
async def get_init_data(app: Client, bot_username: str, start_param: str) -> str:
    ...
    # pakai raw value langsung, jangan di-decode
    raw = params.get("tgWebAppData", "")

    # inject start_param kalau belum ada (dalam bentuk encoded)
    if "start_param" not in raw:
        raw = raw + f"&start_param={urllib.parse.quote(start_param)}"
