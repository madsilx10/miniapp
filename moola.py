...
# ── API calls ────────────────────────────────────────────────────────────────
async def api_post(session, url: str, headers: dict, body: dict = {}) -> dict:
...
async def api_get(session, url: str, headers: dict) -> dict:
...
async def process_account(session_str: str, idx: int, total: int):
    ...
    async with app:
        ...
        async with aiohttp.ClientSession() as http:
            ...
            # stats warm-up
            stats = await api_get(http, f"{BASE_URL}/api/stats", headers)
            print(f"  [*] totalUsers: {stats.get('totalUsers', '?')}")

            # me
            me_data = await api_post(http, f"{BASE_URL}/api/me", headers, {})
            ...
            if not user.get("onboarded"):
                print("  [*] Onboard...")
                res = await api_post(http, f"{BASE_URL}/api/onboard", headers, {})
            ...
            for task_id in TASKS:
                if task_id in social_done:
                    ...
                    continue
                res = await api_post(http, f"{BASE_URL}/api/tasks/social", headers, {"taskId": task_id})
                new_done = res.get("user", {}).get("socialDone", [])
                print(f"  [+] {task_id}: socialDone={new_done} | credited={res.get('credited')}")
                await asyncio.sleep(3)

            # re-check
            me_data2 = await api_post(http, f"{BASE_URL}/api/me", headers, {})
            ...
            if mining.get("active"):
                ...
                res = await api_post(http, f"{BASE_URL}/api/mine/start", headers, {})
