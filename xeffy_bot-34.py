...
async def main():
    ...
    if mode_input == "A":
        ...
        # Cek status airdrop
        idx = int(input(f"Cek akun ke- (1-{total}): ")) - 1
        init_data = sessions[idx]
        session_token = xeffy_login(init_data)
        if session_token:
            status = check_airdrop_status(session_token)
            if status:
                ...
                if reg and reg.get("discordHandle"):
                    print(f"\n✅ Akun {idx+1} sudah register airdrop")
                    print(f"   Discord: {reg.get('discordHandle')}")
                    print(f"   Wallet: {reg.get('walletAddress')}")
                else:
                    print(f"\n❌ Akun {idx+1} belum register airdrop")
            else:
                print(f"\n❌ Gagal cek status akun {idx+1}")
        return
    else:
