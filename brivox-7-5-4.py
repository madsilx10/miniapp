...
# ── Track akun yang udah pernah /start (biar gak spam ulang tiap mode full) ────
STARTED_FILE = "started.txt"

def load_started() -> set:
    if not os.path.exists(STARTED_FILE):
        return set()
    return {l.strip() for l in open(STARTED_FILE).readlines() if l.strip()}

def mark_started(idx: int):
...
async def get_init_data(session_string: str, idx: int, send_start: bool = False) -> str | None:
    ...
    try:
        ...
        # Start bot dulu (register/ref) — cuma perlu sekali seumur hidup per akun
        if send_start:
            try:
...
# ── API: Dividend Claim ────────────────────────────────────────────────────────
async def claim_dividend(token: str, uid: str, idx: int) -> bool:
    try:
        data = await http_get(f"{BASE_URL}/api/deal/user/dividend/settlement", build_headers(token, uid))
        if data.get("msg") == "Success":
            ok(f"ACC#{idx}", "Dividend claim: sukses ✓")
            return True
        else:
            warn(f"ACC#{idx}", f"Dividend claim: {data}")
            return False
    except Exception as e:
        err(f"ACC#{idx}", f"Dividend claim error: {e}")
        return False

# ── API: Daily Check-in ─────────────────────────────────────────────────────────
async def do_checkin(token: str, uid: str, idx: int) -> dict | None:
    try:
        data = await http_post(f"{BASE_URL}/api/user/user/sign-in/create", build_headers(token, uid), {})
        prize = data.get("prize")
        if prize:
            ok(f"ACC#{idx}", f"Check-in → dapat: {prize['number']} {prize['type']} (times: {data.get('times')}) 🎉")
            return {"number": prize["number"], "type": prize["type"]}
        elif data.get("error") == "Signed in today":
            info(f"ACC#{idx}", "Check-in: udah check-in hari ini ✓ (skip)")
            return None
        else:
            warn(f"ACC#{idx}", f"Check-in: {data}")
            return None
    except Exception as e:
        err(f"ACC#{idx}", f"Check-in error: {e}")
        return None
...
async def process_account(session_string: str, idx: int, action: str) -> dict:
    ...
    already_started = idx in load_started()
    init_data = await get_init_data(session_string, idx, send_start=(action == "full" and not already_started))
    if not init_data:
    ...
    # ── Mode: Dividend Claim ────────────────────────────────────────────────
    if action == "dividend":
        await claim_dividend(token, uid, idx)
        await asyncio.sleep(DELAY)
        await get_balance(token, uid, idx)
        return rewards

    # ── Mode: Daily Check-in (loop tiap 5 detik per create sampai habis) ────
    if action in ("checkin", "claim_daily"):
        if action == "claim_daily":
            await claim_dividend(token, uid, idx)
            await asyncio.sleep(DELAY)

        CHECKIN_DELAY = 5
        MAX_TRY = 10  # safety cap biar gak infinite loop kalau API selalu balikin prize
        for attempt in range(MAX_TRY):
            result = await do_checkin(token, uid, idx)
            if result:
                rewards[result["type"]] = rewards.get(result["type"], 0) + float(result["number"])
                await asyncio.sleep(CHECKIN_DELAY)
            else:
                break  # gagal / udah habis jatah check-in hari ini
        await get_balance(token, uid, idx)
        if rewards:
...
async def main():
    ...
    print("  3) Dividend Claim")
    print("  4) Daily Check-in")
    print("  5) Dividend Claim + Daily Check-in")
    action_input = input("\nPilihan (1/2/3/4/5): ").strip()
    action_map = {"1": "full", "2": "spin_only", "3": "dividend", "4": "checkin", "5": "claim_daily"}
    ...
    action_label = {
        "full": "Buy NFT + Spin",
        "spin_only": "Spin aja",
        "dividend": "Dividend Claim",
        "checkin": "Daily Check-in",
        "claim_daily": "Dividend Claim + Daily Check-in",
    }[action]
    print(f"\n→ Proses {len(targets)} akun, mode: {action_label}\n")
