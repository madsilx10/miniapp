...
async def run_predict_manual(session_string, idx, total, manual_scores, match_override=None):
    ...
    for m in matches:
        ...
        home, away = m["home"]["code"], m["away"]["code"]
        key = f"{home}-{away}"
        hs, as_ = manual_scores.get(key, (1, 0))
        res = predict(access_token, m["condition_id"], hs, as_)
        print(f"{tag} predict {home} {hs}-{as_} {away} -> {'OK' if res.get('success') else res}")
        predicted += 1
    if predicted == 0:
...
async def select_mode_and_accounts():
    ...
    manual_scores = {}
    if mode == "7":
        # Ambil daftar match buat input manual
        first_session = SESSIONS[0]
        token_data = await get_token(first_session, 1, n)
        first_token = token_data[0]
        all_matches = [m for m in get_matches(first_token)
                       if m["status"] == "UPCOMING" and m["pool"]["status"] == "OPEN"]
        targets = match_override if match_override else all_matches
        print("\nInput skor tiap match (Enter = 1-0):")
        for m in targets:
            home, away = m["home"]["code"], m["away"]["code"]
            raw = input(f"  {home} vs {away} (contoh: 2-1): ").strip()
            if raw and "-" in raw:
                parts = raw.split("-")
                try:
                    ...
                    print(f"  Format salah, pakai 1-0")
                    manual_scores[f"{home}-{away}"] = (1, 0)
            else:
                manual_scores[f"{home}-{away}"] = (1, 0)
