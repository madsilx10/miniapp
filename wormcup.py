...
_GOAL_WEIGHTS_FAV  = [15, 35, 30, 15, 3, 1, 1, 0]  # favorit: sering 0-3
_GOAL_WEIGHTS_UNDF = [30, 35, 20, 10, 3, 1, 1, 0]  # underdog: sering 0-2
...
def decide_score(match):
    ...
    if PREDICT_MODE == "simple":
        ...
        if dist["home_pct"] >= dist["away_pct"]:
            score = (_weighted_goal(_GOAL_WEIGHTS_FAV), _weighted_goal(_GOAL_WEIGHTS_UNDF))
        else:
            score = (_weighted_goal(_GOAL_WEIGHTS_UNDF), _weighted_goal(_GOAL_WEIGHTS_FAV))
...
async def run_predict_only(session_string, idx, total):
    tag = f"[Akun {idx}/{total}]"
    print(f"\n{'='*40}\n{tag} predict doang")
    access_token, user_id, tag, _ = await get_token(session_string, idx, total)
    ...
    for idx, s in indexed:
        try:
            if mode == "1":
                ...
                await run_predict_only(s, idx, total)
