...
def decide_score(match):
    ...
    if PREDICT_MODE == "simple":
        ...
        score = (1, 0) if dist["home_pct"] >= dist["away_pct"] else (0, 1)
    else:
        # 0-4 sering, 5-6 jarang tapi bisa
        home_score = random.choices([0,1,2,3,4,5,6], weights=[15,25,25,20,10,3,2], k=1)[0]
        away_score = random.choices([0,1,2,3,4,5,6], weights=[15,25,25,20,10,3,2], k=1)[0]
...
def select_predict_mode():
    ...
    print("  2. Simple (selalu 1-0)")
    print("  3. Favored (ikutin distribusi)")
...
def select_mode_and_accounts():
    ...
    if mode in ("1", "6"):
        ...
        if mc == "2":
            ...
            token_data = _asyncio.get_event_loop().run_until_complete(
                get_token(first_session, 1, n)
            )
...
async def main():
    ...
    mode, boost_points, indexed, match_override = select_mode_and_accounts()
    total = len(indexed)
