...
_GOAL_WEIGHTS_FAV  = [10, 28, 30, 20, 7, 3, 1, 1]  # favorit: sering 1-3
_GOAL_WEIGHTS_UNDF = [25, 30, 25, 12, 5, 2, 1, 0]  # underdog: sering 0-2
...
async def run_boost(session_string, idx, total, points):
    ...
    for m in matches:
        ...
        if not p:
            continue
        print(f"{tag} debug predict: status={p.get('status')} boost_multiplier={p.get('boost_multiplier')} match={m['home']['code']}vs{m['away']['code']}")
...
async def run_cek_hasil(session_string, idx, total):
    ...
    for m in matches:
        ...
        if status == "WON":
            payout = p.get("payout_usdc") or "?"
            print(f"{tag} ✅ WON  {home} {hs}-{as_} {away}{bstr} | +${payout} USDC")
            ada_hasil = True
        elif status == "LOST":
            result = m.get("result")
            actual = f"{result['home_score']}-{result['away_score']}" if result else "?"
            print(f"{tag} ❌ LOST {home} {hs}-{as_} {away}{bstr} | hasil: {actual}")
            ada_hasil = True
        elif status == "PENDING":
            print(f"{tag} ⏳ PENDING {home} {hs}-{as_} {away}{bstr}")
            ada_hasil = True
    if not ada_hasil:
...
async def run_ubah_predict(session_string, idx, total, changes):
    """changes = list of (condition_id, home_score, away_score)"""
    tag = f"[Akun {idx}/{total}]"
    print(f"\n{'='*40}\n{tag} ubah predict")
    access_token, user_id, tag, _ = await get_token(session_string, idx, total)

    for condition_id, hs, as_ in changes:
        res = predict(access_token, condition_id, hs, as_)
        print(f"{tag} ubah predict {condition_id[:16]}... {hs}-{as_} -> {'OK' if res.get('success') else res}")


# ===================== ESPN LIVE SCORES =====================
ESPN_TEAM_MAP = {
    # ESPN team abbreviation -> WormCup team code
    # ESPN biasanya pakai 3 huruf yang sama, tapi beberapa beda
    "USA": "USA", "PAR": "PAR", "QAT": "QAT", "SUI": "SUI",
    "BRA": "BRA", "MAR": "MAR", "CAN": "CAN", "BIH": "BOS",
    "KOR": "KOR", "CZE": "CZE", "GER": "GER", "NED": "NED",
    "ARG": "ARG", "MEX": "MEX", "ENG": "ENG", "FRA": "FRA",
    "ESP": "ESP", "POR": "POR", "ITA": "ITA", "JPN": "JPN",
}

def get_espn_live_scores():
    """Ambil semua match yang lagi live dari ESPN."""
    try:
        r = requests.get(
        ...
        for event in data.get("events", []):
            ...
            status = comp["status"]
            stype  = status["type"]["name"]
            clock  = status.get("clock", 0)  # menit (float)
            period = status.get("period", 1)

            if stype not in ("STATUS_IN_PROGRESS",):
                continue

            home = comp["competitors"][0]
            away = comp["competitors"][1]

            # ESPN pakai home index 0 (home=True)
            if not home.get("homeAway") == "home":
                home, away = away, home
...
def match_espn_to_wormcup(espn_game, wormcup_matches):
    """Cari match WormCup yang cocok dengan ESPN game berdasarkan kode tim."""
    eh = ESPN_TEAM_MAP.get(espn_game["home_abbr"], espn_game["home_abbr"])
    ea = ESPN_TEAM_MAP.get(espn_game["away_abbr"], espn_game["away_abbr"])

    for m in wormcup_matches:
        wh = m["home"]["code"]
        wa = m["away"]["code"]
        if (wh == eh and wa == ea) or (wh == ea and wa == eh):
            return m
    return None


async def run_live_predict(indexed, total):
    """
    Mode auto live predict:
    - Loop tiap 60 detik
    - Cek ESPN, kalau match menit 85+ → update predict semua akun
    - Berhenti kalau semua match udah di-update atau gak ada match live
    """
    print("\n[LIVE] mulai monitor skor ESPN...")
    updated = set()  # condition_id yang udah di-update

    while True:
        espn_games = get_espn_live_scores()

        if not espn_games:
            print("[LIVE] gak ada match live sekarang, cek lagi 60 detik...")
            time.sleep(60)
            continue
            clock = game["clock"]
            period = game["period"]
...
def select_mode_and_accounts():
    ...
    print("  6. Ubah predict (input skor manual)")
    print("  7. Auto live predict (update skor di menit 85+)")
    mode = input("Pilih mode (1/2/3/4/5/6/7): ").strip()
    ...
    if mode == "6":
        print("\nMasukkan skor baru per match (condition_id bisa dicek di cek hasil)")
        print("Ketik 'selesai' kalau udah")
        while True:
            cid = input("condition_id (atau 'selesai'): ").strip()
            if cid.lower() == "selesai":
                break
            hs  = int(input("Home score: ").strip())
            as_ = int(input("Away score: ").strip())
            changes.append((cid, hs, as_))
    ...
    return mode, boost_points, changes, indexed
...
async def main():
    ...
    mode, boost_points, changes, indexed = select_mode_and_accounts()
    total = len(indexed)
    ...
    for idx, s in indexed:
        try:
            if mode == "1":
                ...
                await run_ubah_predict(s, idx, total, changes)
        except Exception as e:
            print(f"[Akun {idx}] Error: {e}")

    if mode == "7":
        await run_live_predict(indexed, total)
