...
async def run_normal(session_string, idx, total, match_override=None):
    ...
    for m in matches:
        ...
        if tier is None or tier.get("my_prediction") is not None:
            continue
        hs, as_ = decide_score(m)
        if tier.get("tier") == "JACKPOT":
            jersey = random.randint(1, 99)
            res  = predict_jackpot(access_token, tier["condition_id"], hs, as_, jersey)
        else:
            res  = predict(access_token, tier["condition_id"], hs, as_)
        debug_jersey_error(m, tier, res)
        home, away = m["home"]["code"], m["away"]["code"]
        jersey_info = f" (jersey={jersey})" if tier.get("tier") == "JACKPOT" else ""
        print(f"{tag} predict {home} {hs}-{as_} {away}{jersey_info} -> {'OK' if res.get('success') else res}")
        predicted += 1
    if predicted == 0:
...
async def run_predict_manual(session_string, idx, total, manual_scores, match_override=None):
    ...
    for m in matches:
        ...
        if key in manual_scores:
            ...
            hs, as_ = decide_score(m)
        if tier.get("tier") == "JACKPOT":
            jersey_raw = input(f"  Jersey pencetak gol pertama {home} vs {away} (nomor punggung): ").strip()
            jersey = int(jersey_raw) if jersey_raw.isdigit() else random.randint(1, 99)
            res = predict_jackpot(access_token, tier["condition_id"], hs, as_, jersey)
        else:
            res = predict(access_token, tier["condition_id"], hs, as_)
        debug_jersey_error(m, tier, res)
