...
async def select_mode_and_accounts():
    ...
    if mode == "3":
        ...
        for p in predictions:
            ...
            label = f'{m["home"]["code"]} vs {m["away"]["code"]}' if m else "?"
            print(f"  - {label} | status={p['status']} | boost_multiplier={p.get('boost_multiplier')}")

        boostable = [p for p in predictions if p["status"] == "PENDING" and p["boost_multiplier"] == 1]
