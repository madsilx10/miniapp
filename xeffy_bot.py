...
def register_airdrop(session_token, discord_handle, wallet_address, x_handle=None):
    ...
    print(f"  [DEBUG] register_airdrop: {r.status_code} | {r.text[:300]}")
    return r.status_code in [200, 201]
...
# ── Core logic per akun ──────────────────────────────────
async def run_account(session_string, index, tele_links, mode="full", airdrop_data=None, xhandles=None):
    ...
    if mode == "airdrop":
        ...
        if status and status.get("registration") is not None:
        ...
        # Verify X follow
        if x_handle:
            print(f"[Akun {index}] Verify X follow @{x_handle}...")
            ok = verify_x_follow(session_token, x_handle)
            if not ok:
                print(f"[Akun {index}] ❌ X follow verify gagal")
                return
            print(f"[Akun {index}] ✅ X follow verified")
...
async def main():
    ...
    mode_input = input("Pilih mode (A/B/C/D/E): ").strip().upper()
    
    if mode_input == "E":
        # Cek status airdrop untuk akun yang dipilih
        for i in indices:
            init_data = sessions[i]
            session_token = xeffy_login(init_data)
            if session_token:
                status = check_airdrop_status(session_token)
                if status:
                    reg = status.get("registration")
                    if reg and reg.get("discordHandle"):
                        print(f"[Akun {i+1}] ✅ Sudah register - Discord: {reg.get('discordHandle')}, Wallet: {reg.get('walletAddress')[:10]}...")
                    else:
                        print(f"[Akun {i+1}] ❌ Belum register")
                else:
                    print(f"[Akun {i+1}] ❌ Gagal cek status")
            else:
                print(f"[Akun {i+1}] ❌ Login gagal")
    ...
    for i in indices:
        await run_account(sessions[i], i + 1, tele_links, mode=mode, airdrop_data=airdrop_data, xhandles=xhandles)
        if i != indices[-1]:
