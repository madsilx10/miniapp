...
# ── Core logic per akun ──────────────────────────────────
async def run_account(session_string, index, tele_links, mode="full", airdrop_data=None):
...
async def main():
    ...
    for i in indices:
        await run_account(sessions[i], i + 1, tele_links, mode=mode, airdrop_data=airdrop_data)
        if i != indices[-1]:
