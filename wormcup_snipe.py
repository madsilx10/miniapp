...
# ===================== POLLING =====================
async def polling_loop(tokens_per_acc):
    ...
    while True:
        try:
            ...
            for m in matches:
                ...
                await do_snipe(m, tokens_per_acc)
...
async def main():
    ...
    print("Predict mode:")
    print("  1. Random")
    print("  2. Favored (favorit cenderung menang)")
    pm = input("Pilih (1/2) [Enter=random]: ").strip()
    PREDICT_MODE = "favored" if pm == "2" else "random"
    print(f"Mode: {PREDICT_MODE}\n")

    total          = len(SESSIONS)
    tokens_per_acc = []
    print(f"⏳ Login {total} akun...")
    ...
    await polling_loop(tokens_per_acc)
