...
async def do_snipe(match, tokens_per_acc, scores_queue):
    ...
    if scores_queue:
        ...
        print(f"  Skor dari file: {hs}-{as_} jersey={jersey_fixed}")
    else:
        _total_w = [8, 20, 25, 22, 12, 7, 4, 2]
        total    = random.choices(range(8), weights=_total_w, k=1)[0]
        hs       = random.randint(0, total)
        as_      = total - hs
        jersey_fixed = random.randint(1, 99)
        print(f"  Skor file habis, pakai random: {hs}-{as_} jersey={jersey_fixed}")
    ...
    for tag, token in tokens_per_acc:
        for t in tiers:
            ...
            if tier_name == "JACKPOT":
                res = predict_jackpot(token, cid, hs, as_, jersey_fixed)
                debug_jersey_error(match, t, res)
                print(f"  {tag} [JACKPOT] {home} {hs}-{as_} {away} jersey={jersey_fixed} -> {'OK' if res.get('success') else res}")
